...
def main():
    ...
    for cycle in range(1,cycles+1):
        ...
        uid=str(rows[0]["uid"]);fn=Path(str(store.get_filename(uid)));fn.write_text('{"transactions":[{"value":125.5,"description":"Grant"},{"value":-25.5,"description":"Supplies"}]}\n',encoding="utf-8");rpid,raid,rnode=launch(uid,"Balance: 100.00");assert "Balance: 100.00" in Atspi.Text.get_text(rnode,0,-1);stop(rpid,raid);assert "transactions" in fn.read_text(encoding="utf-8")
