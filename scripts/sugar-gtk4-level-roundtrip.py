#!/usr/bin/env python3
"""Guest regression: restore a Level inclination from its Journal object."""
import os, subprocess, sys, time
from pathlib import Path
BUNDLE_ID="net.flossmanuals.LevelActivity"; MARKER="levelactivity4.LevelActivity"
...
def main():
    ...
    def find(node,pid,text,depth=0):
        ...
        return None
    def launch(uid="",expected="+0°"):
        assert journal.LaunchBundle(BUNDLE_ID,uid);pid,aid=wait_for("Level process",lambda:next(iter(procs()),None));wait_for("service",lambda:bus.name_has_owner("org.laptop.Activity"+aid));wait_for("shell",lambda:shell.ActivateActivity(aid));return pid,aid,wait_for("visible level",lambda:find(Atspi.get_desktop(0),pid,expected))
    def stop(pid,aid):
    ...
    if procs():raise SystemExit("Level already running")
    ...
    for cycle in range(1,cycles+1):
        ...
        uid=str(rows[0]["uid"]);fn=Path(str(store.get_filename(uid)));fn.write_text('{"angle": -17}\n',encoding="utf-8");rpid,raid,rnode=launch(uid,"-17°");assert "-17° inclination"==Atspi.Text.get_text(rnode,0,-1);stop(rpid,raid);print(f"cycle={cycle} pid={pid} resumed_pid={rpid} object={uid} resume=PASS service-release=PASS shell-cleanup=PASS",flush=True)
    print("level-roundtrip=PASS input-method=AT-SPI datastore-payload=seeded",flush=True)
