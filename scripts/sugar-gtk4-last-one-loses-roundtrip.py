#!/usr/bin/env python3
"""Guest regression: resume Last One Loses pile from Journal."""
import json, os, subprocess, sys, time
from pathlib import Path
BUNDLE_ID = "org.olpc-france.LOLActivity"; MARKER = "lolactivity4.LastOneLosesActivity"
...
def main():
    ...
    def find(node, pid, expected, depth=0):
        ...
        return None
    def launch(uid="", expected="Last One Loses"):
        assert journal.LaunchBundle(BUNDLE_ID, uid); pid, aid = wait_for("process", lambda: next(iter(procs()), None)); wait_for("service", lambda: bus.name_has_owner("org.laptop.Activity" + aid)); assert shell.ActivateActivity(aid); return pid, aid, wait_for("visible Last One Loses", lambda: find(Atspi.get_desktop(0), pid, expected))
    def stop(pid, aid):
    ...
    uid = str(rows[0]["uid"]); filename = Path(str(store.get_filename(uid))); filename.write_text(json.dumps({"pile": 7}) + "\n")
    pid, aid, node = launch(uid, "7 tokens remain"); assert "7 tokens remain" in Atspi.Text.get_text(node, 0, -1); stop(pid, aid)
    print("last-one-loses-roundtrip=PASS resume=PASS cleanup=PASS", flush=True)
