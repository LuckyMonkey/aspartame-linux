...
for patch in "$patch_dir"/*.patch; do
    ...
    case "$patch" in
        ...
        *0145*) target="$root/sources/sugar" ;;
        *0146*) target="$root/sources/sugar" ;;
        *0147*) target="$toolkit" ;;
        *0148*) target="$toolkit" ;;
        *0149*) target="$toolkit" ;;
        *0150*) target="$toolkit" ;;
        ...
        *0153*) target="$root/sources/sugar" ;;
        ...
        *0159*) target="$root/sources/sugar" ;;
        *0160*) target="$root/sources/sugar" ;;
        *0161*) target="$root/sources/sugar" ;;
    ...
    if [[ "$patch_name" == *0129* ]] &&
        (cd "$shell" && patch --dry-run --fuzz=3 -p1 < "$patch" >/dev/null 2>&1); then
        (cd "$shell" && patch --fuzz=3 -p1 < "$patch" >/dev/null)
        printf '%s\n' "$patch_digest" > "$stamp" 2>/dev/null || true
        echo "applied Journal rooted/unparented attach patch: $patch_name"
        continue
    fi
    if [[ "$patch_name" == *0129* ]] &&
    ...
    # probe's deliberate removal as satisfying it too.
    if [[ "$patch_name" == *0044* ]] &&
        grep -qe 'logging.warning("GTK4 semantic key event: %s", key)' \
                -e 'Key dispatch is deliberately not logged' \
                "$shell/src/jarabe/view/keyhandler.py" 2>/dev/null; then
        printf '%s\n' "$patch_digest" > "$stamp" 2>/dev/null || true
    ...
    if [ -f "$stamp" ] &&
        ...
        grep -q 'self.update_property.*Gtk.AccessibleProperty.LABEL' \
            "$shell/src/jarabe/frame/framewindow.py" &&
        grep -q '^            rect_width = style.LINE_WIDTH' \
            "$shell/src/jarabe/frame/framewindow.py"; then
        python3 - "$shell/src/jarabe/frame/framewindow.py" <<'PY'
...
text = text.replace(
    ...
    "")
needle = "    def __init__(self, position):\n        super().__init__()\n"
replacement = (needle +
               "        self.update_property([Gtk.AccessibleProperty.LABEL], [_('Frame')])\n"+
               "        self.set_accessible_role(Gtk.AccessibleRole.GROUP)\n")
if needle not in text:
    raise SystemExit('FrameContainer initializer not found')
path.write_text(text.replace(needle, replacement, 1))
PY
        printf "%s\n" "$patch_digest" > "$stamp"
        echo "repaired Frame accessibility placement semantically: $patch_name"
    elif [[ "$patch_name" == *0119* ]] &&
        (cd "$target" && patch --dry-run --fuzz=5 -p1 < "$patch" >/dev/null 2>&1); then
        (cd "$target" && patch --fuzz=5 -p1 < "$patch" >/dev/null)
        printf "%s\n" "$patch_digest" > "$stamp"
        echo "repaired Frame accessibility placement: $patch_name"
    elif [[ "$patch_name" == *0162* ]] &&
        grep -q 'panel._has_modal = False' "$shell/src/jarabe/view/service.py"; then
        printf "%s\n" "$patch_digest" > "$stamp"
        echo "verified existing control panel modal state: $patch_name"
    elif [[ "$patch_name" == *0162* ]] &&
        grep -q 'def _close_control_panel' "$shell/src/jarabe/view/service.py" &&
        ! grep -q 'panel._has_modal = False' "$shell/src/jarabe/view/service.py"; then
        python3 - "$shell/src/jarabe/view/service.py" <<'PY'
from pathlib import Path
import sys

path = Path(sys.argv[1])
text = path.read_text()
needle = "            self._control_panel = None\n"
replacement = (needle +
               "            if getattr(panel, '_has_modal', False):\n"+
               "                panel._has_modal = False\n"+
               "                self._shell_model.pop_modal()\n")
if needle not in text:
    raise SystemExit('control panel close state not found')
path.write_text(text.replace(needle, replacement, 1))
PY
        printf "%s\n" "$patch_digest" > "$stamp"
...
ballandbrick_activity="$repo/packages/gtk4-ballandbrick-activity"
test -f "$ballandbrick_activity/activity/activity.info" || { echo "missing native GTK4 BallAndBrick Activity bundle" >&2; exit 2; }
ln -sfn "$ballandbrick_activity" "$activity_dir/BallAndBrick.activity"
...
maze_activity="$repo/packages/gtk4-maze-activity"
test -f "$maze_activity/activity/activity.info" || { echo "missing native GTK4 Maze Activity bundle" >&2; exit 2; }
ln -sfn "$maze_activity" "$activity_dir/Maze.activity"
fototoon_activity="$repo/packages/gtk4-fototoon-activity"
test -f "$fototoon_activity/activity/activity.info" || { echo "missing native GTK4 FotoToon Activity bundle" >&2; exit 2; }
ln -sfn "$fototoon_activity" "$activity_dir/FotoToon.activity"
...
finance_activity="$repo/packages/gtk4-finance-activity"
test -f "$finance_activity/activity/activity.info" || { echo "missing native GTK4 Finance Activity bundle" >&2; exit 2; }
ln -sfn "$finance_activity" "$activity_dir/Finance.activity"
words_activity="$repo/packages/gtk4-words-activity"
test -f "$words_activity/activity/activity.info" || { echo "missing native GTK4 Words Activity bundle" >&2; exit 2; }
ln -sfn "$words_activity" "$activity_dir/Words.activity"
...
# Terminal and Browse/Help are native GTK4 activities; retired VTE/WebKit
# previews are no longer part of the modern runtime, so do not require those
# optional GTK3-era packages here.
