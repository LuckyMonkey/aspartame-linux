#!/usr/bin/env python3
"""Guest regression: resume Appel-Haken colour configuration from Journal."""
import json, os, subprocess, sys, time
from pathlib import Path
BUNDLE_ID = "mulawa.AppelHaken"; MARKER = "appelhakenactivity4.AppelHakenActivity"
...
def main():
    ...
    def find(node, pid, expected, depth=0):
        ...
        return None
    def launch(uid="", expected="Appel Haken"):
        assert journal.LaunchBundle(BUNDLE_ID, uid); pid, aid = wait_for("process", lambda: next(iter(procs()), None)); wait_for("service", lambda: bus.name_has_owner("org.laptop.Activity" + aid)); assert shell.ActivateActivity(aid); return pid, aid, wait_for("visible Appel Haken", lambda: find(Atspi.get_desktop(0), pid, expected))
    def stop(pid, aid):
    ...
    uid = str(rows[0]["uid"]); filename = Path(str(store.get_filename(uid))); filename.write_text(json.dumps({"values": [3, 2, 1, 0]}) + "\n")
    pid, aid, node = launch(uid, "Solved!"); assert "Solved!" in Atspi.Text.get_text(node, 0, -1); stop(pid, aid)
    print("appelhaken-roundtrip=PASS resume=PASS cleanup=PASS", flush=True)
