#!/usr/bin/env python3
"""Guest regression: resume Maze player position from Journal."""
import json, os, subprocess, sys, time
from pathlib import Path
BUNDLE_ID = "vu.lux.olpc.Maze"; MARKER = "mazeactivity4.MazeActivity"
...
def main():
    ...
    def find(node, pid, expected, depth=0):
        ...
        return None
    def launch(uid="", expected="Maze"):
        assert journal.LaunchBundle(BUNDLE_ID, uid); pid, aid = wait_for("process", lambda: next(iter(procs()), None)); wait_for("service", lambda: bus.name_has_owner("org.laptop.Activity" + aid)); assert shell.ActivateActivity(aid); return pid, aid, wait_for("visible Maze", lambda: find(Atspi.get_desktop(0), pid, expected))
    def stop(pid, aid):
    ...
    uid = str(rows[0]["uid"]); filename = Path(str(store.get_filename(uid))); filename.write_text(json.dumps({"position": [2, 1]}) + "\n")
    pid, aid, node = launch(uid, "Position: 3, 2"); assert "Position: 3, 2" in Atspi.Text.get_text(node, 0, -1); stop(pid, aid)
    print("maze-roundtrip=PASS resume=PASS cleanup=PASS", flush=True)
