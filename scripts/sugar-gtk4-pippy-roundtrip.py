...
def main():
    ...
    for cycle in range(1,cycles+1):
        ...
        uid=str(rows[0]["uid"]);fn=Path(str(store.get_filename(uid)));fn.write_text('print("Aspartame")\n',encoding="utf-8");rpid,raid,rnode=launch(uid,"Aspartame");assert "Aspartame" in Atspi.Text.get_text(rnode,0,-1);stop(rpid,raid);assert "Aspartame" in fn.read_text(encoding="utf-8")
