#!/usr/bin/env python3
"""Guest regression: resume Jumble puzzle index and answer state."""
import json, os, subprocess, sys, time
from pathlib import Path
BUNDLE_ID = "mulawa.Jumble"; MARKER = "jumbleactivity4.JumbleActivity"
...
def main():
    ...
    def find(node, pid, expected, depth=0):
        ...
        return None
    def launch(uid="", expected="Jumble"):
        assert journal.LaunchBundle(BUNDLE_ID, uid); pid, aid = wait_for("process", lambda: next(iter(procs()), None)); wait_for("service", lambda: bus.name_has_owner("org.laptop.Activity" + aid)); assert shell.ActivateActivity(aid); return pid, aid, wait_for("visible Jumble", lambda: find(Atspi.get_desktop(0), pid, expected))
    def stop(pid, aid):
    ...
    uid = str(rows[0]["uid"]); filename = Path(str(store.get_filename(uid))); filename.write_text(json.dumps({"index": 1, "attempts": 2, "answer": "ACTIVITY"}) + "\n")
    pid, aid, node = launch(uid, "Word 2 of 3"); assert "Word 2 of 3" in Atspi.Text.get_text(node, 0, -1); stop(pid, aid)
    print("jumble-roundtrip=PASS resume=PASS cleanup=PASS", flush=True)
