#!/usr/bin/env python3
"""Guest regression: restore a Jukebox playlist from its Journal object."""
import os, subprocess, sys, time
from pathlib import Path
BUNDLE_ID="org.laptop.sugar.Jukebox"; MARKER="jukeboxactivity4.JukeboxActivity"
...
def main():
    ...
    def find(node,pid,text,depth=0):
        ...
        return None
    def launch(uid="",expected="Jukebox"):
        assert journal.LaunchBundle(BUNDLE_ID,uid);pid,aid=wait_for("Jukebox process",lambda:next(iter(procs()),None));wait_for("service",lambda:bus.name_has_owner("org.laptop.Activity"+aid));wait_for("shell",lambda:shell.ActivateActivity(aid));return pid,aid,wait_for("visible Jukebox",lambda:find(Atspi.get_desktop(0),pid,expected))
    def stop(pid,aid):
    ...
    if procs():raise SystemExit("Jukebox already running")
