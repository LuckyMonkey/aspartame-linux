#!/usr/bin/env python3
"""Guest regression: resume Across and Down clue/letters from Journal."""
import json, os, subprocess, sys, time
from pathlib import Path
BUNDLE_ID = "mulawa.AcrossDown"; MARKER = "acrossdownactivity4.AcrossDownActivity"
...
def main():
    ...
    def find(node, pid, expected, depth=0):
        ...
        return None
    def launch(uid="", expected="Across and Down"):
        assert journal.LaunchBundle(BUNDLE_ID, uid); pid, aid = wait_for("process", lambda: next(iter(procs()), None)); wait_for("service", lambda: bus.name_has_owner("org.laptop.Activity" + aid)); assert shell.ActivateActivity(aid); return pid, aid, wait_for("visible Across and Down", lambda: find(Atspi.get_desktop(0), pid, expected))
    def stop(pid, aid):
    ...
    uid = str(rows[0]["uid"]); filename = Path(str(store.get_filename(uid))); filename.write_text(json.dumps({"index": 1, "selected": 3, "letters": list("JOURNAL")}) + "\n")
    pid, aid, node = launch(uid, "Where Sugar saves work"); clue = Atspi.Text.get_text(node, 0, -1); assert "Where Sugar saves work" in clue; stop(pid, aid)
    print("acrossdown-roundtrip=PASS resume=PASS cleanup=PASS", flush=True)
