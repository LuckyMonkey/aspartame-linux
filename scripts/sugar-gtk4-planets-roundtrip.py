#!/usr/bin/env python3
"""Guest regression: restore selected planet from Journal."""
import os, subprocess, sys, time
from pathlib import Path
BUNDLE_ID="org.sugarlabs.Planets"; MARKER="planetsactivity4.PlanetsActivity"
...
def main():
    ...
    def find(node,pid,text,depth=0):
        ...
        return None
    def launch(uid="",expected="Planets"):
        assert journal.LaunchBundle(BUNDLE_ID,uid);pid,aid=wait_for("Planets process",lambda:next(iter(procs()),None));wait_for("service",lambda:bus.name_has_owner("org.laptop.Activity"+aid));wait_for("shell",lambda:shell.ActivateActivity(aid));return pid,aid,wait_for("visible Planets",lambda:find(Atspi.get_desktop(0),pid,expected))
    def stop(pid,aid):
    ...
    if procs():raise SystemExit("Planets already running")
    ...
    for cycle in range(1,cycles+1):
        ...
        uid=str(rows[0]["uid"]);fn=Path(str(store.get_filename(uid)));fn.write_text('{"selected":"Mars"}\n',encoding="utf-8");rpid,raid,rnode=launch(uid,"Mars — selected planet");assert "Mars — selected planet" in Atspi.Text.get_text(rnode,0,-1);stop(rpid,raid);assert "Mars" in fn.read_text(encoding="utf-8")
        print(f"cycle={cycle} pid={pid} resumed_pid={rpid} object={uid} resume=PASS service-release=PASS shell-cleanup=PASS",flush=True)
    print("planets-roundtrip=PASS input-method=AT-SPI datastore-payload=seeded",flush=True)
