#!/usr/bin/env python3
"""Guest regression: restore Game of Life grid state from Journal."""
import os, subprocess, sys, time
from pathlib import Path
BUNDLE_ID="org.sugarlabs.gameOfLife"; MARKER="gameoflifeactivity4.GameOfLifeActivity"
...
def main():
    ...
    def find(node,pid,text,depth=0):
        ...
        return None
    def launch(uid="",expected="Generation: 0"):
        assert journal.LaunchBundle(BUNDLE_ID,uid);pid,aid=wait_for("Game of Life process",lambda:next(iter(procs()),None));wait_for("service",lambda:bus.name_has_owner("org.laptop.Activity"+aid));wait_for("shell",lambda:shell.ActivateActivity(aid));return pid,aid,wait_for("visible generation",lambda:find(Atspi.get_desktop(0),pid,expected))
    def stop(pid,aid):
    ...
    if procs():raise SystemExit("Game of Life already running")
    ...
    for cycle in range(1,cycles+1):
        ...
        uid=str(rows[0]["uid"]);fn=Path(str(store.get_filename(uid)));fn.write_text('{"cells":[[5,5],[5,6],[5,7]],"generation":3}\n',encoding="utf-8");rpid,raid,rnode=launch(uid,"Generation: 3");assert "Generation: 3 · 3 cells" in Atspi.Text.get_text(rnode,0,-1);stop(rpid,raid);assert "generation" in fn.read_text(encoding="utf-8")
        print(f"cycle={cycle} pid={pid} resumed_pid={rpid} object={uid} resume=PASS service-release=PASS shell-cleanup=PASS",flush=True)
    print("gameoflife-roundtrip=PASS input-method=AT-SPI datastore-payload=seeded",flush=True)
