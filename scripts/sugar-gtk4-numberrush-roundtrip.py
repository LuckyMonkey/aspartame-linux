#!/usr/bin/env python3
"""Guest regression: resume Number Rush round and score from Journal."""
import json, os, subprocess, sys, time
from pathlib import Path
BUNDLE_ID = "org.sugarlabs.NumRush"; MARKER = "numberrushactivity4.NumberRushActivity"
...
def main():
    ...
    def find(node, pid, expected, depth=0):
        ...
        return None
    def launch(uid=""):
        assert journal.LaunchBundle(BUNDLE_ID, uid); pid, aid = wait_for("process", lambda: next(iter(procs()), None)); wait_for("service", lambda: bus.name_has_owner("org.laptop.Activity" + aid)); assert shell.ActivateActivity(aid); return pid, aid, wait_for("visible Number Rush", lambda: find(Atspi.get_desktop(0), pid, "Number Rush"))
    def stop(pid, aid):
    ...
    uid = str(rows[0]["uid"]); filename = Path(str(store.get_filename(uid))); filename.write_text(json.dumps({"round": 2, "score": 4, "solved": True, "answer": "15"}) + "\n")
    pid, aid, node = launch(uid); score_node = wait_for("restored score", lambda: find(Atspi.get_desktop(0), pid, "Score: 4")); assert "Score: 4" in Atspi.Text.get_text(score_node, 0, -1); stop(pid, aid)
    print("numberrush-roundtrip=PASS resume=PASS cleanup=PASS", flush=True)
