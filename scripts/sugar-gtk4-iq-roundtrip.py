#!/usr/bin/env python3
"""Guest regression: resume IQ puzzle round from Journal."""
import json, os, subprocess, sys, time
from pathlib import Path
BUNDLE_ID = "mulawa.IQ"; MARKER = "iqactivity4.IQActivity"
...
def main():
    ...
    def find(node, pid, expected, depth=0):
        ...
        return None
    def launch(uid="", expected="IQ"):
        assert journal.LaunchBundle(BUNDLE_ID, uid); pid, aid = wait_for("process", lambda: next(iter(procs()), None)); wait_for("service", lambda: bus.name_has_owner("org.laptop.Activity" + aid)); assert shell.ActivateActivity(aid); return pid, aid, wait_for("visible IQ", lambda: find(Atspi.get_desktop(0), pid, expected))
    def stop(pid, aid):
    ...
    uid = str(rows[0]["uid"]); filename = Path(str(store.get_filename(uid))); filename.write_text(json.dumps({"round": 1}) + "\n")
    pid, aid, node = launch(uid, "Puzzle 2 of 3"); assert "Puzzle 2 of 3" in Atspi.Text.get_text(node, 0, -1); stop(pid, aid)
    print("iq-roundtrip=PASS resume=PASS cleanup=PASS", flush=True)
