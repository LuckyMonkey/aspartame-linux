#!/usr/bin/env python3
"""Guest regression: resume PlayGo board/turn state from Journal."""
import json, os, subprocess, sys, time
from pathlib import Path
BUNDLE_ID = "org.laptop.PlayGo"; MARKER = "playgoactivity4.PlayGoActivity"
...
def main():
    ...
    def find(node, pid, expected, depth=0):
        ...
        return None
    def launch(uid="", expected="PlayGo"):
        assert journal.LaunchBundle(BUNDLE_ID, uid); pid, aid = wait_for("process", lambda: next(iter(procs()), None)); wait_for("service", lambda: bus.name_has_owner("org.laptop.Activity" + aid)); assert shell.ActivateActivity(aid); return pid, aid, wait_for("visible PlayGo", lambda: find(Atspi.get_desktop(0), pid, expected))
    def stop(pid, aid):
    ...
    uid = str(rows[0]["uid"]); filename = Path(str(store.get_filename(uid))); board = [0] * 25; board[0], board[6], board[12] = 1, 2, 1; filename.write_text(json.dumps({"board": board, "turn": 2}) + "\n")
    pid, aid, node = launch(uid, "White to play"); assert "White to play" in Atspi.Text.get_text(node, 0, -1); stop(pid, aid)
    print("playgo-roundtrip=PASS resume=PASS cleanup=PASS", flush=True)
