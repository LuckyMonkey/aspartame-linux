#!/usr/bin/env python3
"""Guest regression: resume Gears rotation from Journal."""
import json, os, subprocess, sys, time
from pathlib import Path
BUNDLE_ID = "org.sugarlabs.GearsActivity"; MARKER = "gearsactivity4.GearsActivity"
...
def main():
    ...
    def find(node, pid, expected, depth=0):
        ...
        return None
    def launch(uid="", expected="Gears"):
        assert journal.LaunchBundle(BUNDLE_ID, uid); pid, aid = wait_for("process", lambda: next(iter(procs()), None)); wait_for("service", lambda: bus.name_has_owner("org.laptop.Activity" + aid)); assert shell.ActivateActivity(aid); return pid, aid, wait_for("visible Gears", lambda: find(Atspi.get_desktop(0), pid, expected))
    def stop(pid, aid):
    ...
    uid = str(rows[0]["uid"]); filename = Path(str(store.get_filename(uid))); filename.write_text(json.dumps({"phase": 1.5}) + "\n")
    pid, aid, node = launch(uid, "Rotation: 1.50 radians"); assert "Rotation: 1.50 radians" in Atspi.Text.get_text(node, 0, -1); stop(pid, aid)
    print("gears-roundtrip=PASS resume=PASS cleanup=PASS", flush=True)
