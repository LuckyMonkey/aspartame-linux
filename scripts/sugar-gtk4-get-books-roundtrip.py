#!/usr/bin/env python3
"""Guest regression: restore Get Books catalog selection from Journal."""
import os, subprocess, sys, time
from pathlib import Path
BUNDLE_ID="org.laptop.sugar.GetBooksActivity"; MARKER="getbooksactivity4.GetBooksActivity"
...
def main():
    ...
    def find(node,pid,text,depth=0):
        ...
        return None
    def launch(uid="",expected="Get Books"):
        assert journal.LaunchBundle(BUNDLE_ID,uid);pid,aid=wait_for("Get Books process",lambda:next(iter(procs()),None));wait_for("service",lambda:bus.name_has_owner("org.laptop.Activity"+aid));wait_for("shell",lambda:shell.ActivateActivity(aid));return pid,aid,wait_for("visible catalog",lambda:find(Atspi.get_desktop(0),pid,expected))
    def stop(pid,aid):
    ...
    if procs():raise SystemExit("Get Books already running")
    ...
    for cycle in range(1,cycles+1):
        ...
        uid=str(rows[0]["uid"]);fn=Path(str(store.get_filename(uid)));fn.write_text('{"query":"Secret","selected":"The Secret Garden"}\n',encoding="utf-8");rpid,raid,rnode=launch(uid,"The Secret Garden");assert "The Secret Garden" in Atspi.Text.get_text(rnode,0,-1);stop(rpid,raid);assert "Secret" in fn.read_text(encoding="utf-8")
        print(f"cycle={cycle} pid={pid} resumed_pid={rpid} object={uid} resume=PASS service-release=PASS shell-cleanup=PASS",flush=True)
    print("get-books-roundtrip=PASS input-method=AT-SPI datastore-payload=seeded",flush=True)
