#!/usr/bin/env python3
"""Guest regression: resume Mastermind guess progress from Journal."""
import json, os, subprocess, sys, time
from pathlib import Path
BUNDLE_ID = "org.laptop.Mastermind"; MARKER = "mastermindactivity4.MastermindActivity"
...
def main():
    ...
    def find(node, pid, expected, depth=0):
        ...
        return None
    def launch(uid="", expected="0 of 6 guesses"):
        assert journal.LaunchBundle(BUNDLE_ID, uid); pid, aid = wait_for("process", lambda: next(iter(procs()), None)); wait_for("service", lambda: bus.name_has_owner("org.laptop.Activity" + aid)); assert shell.ActivateActivity(aid); return pid, aid, wait_for("visible progress", lambda: find(Atspi.get_desktop(0), pid, expected))
    def stop(pid, aid):
    ...
    pid, aid, node = launch(); assert "0 of 6 guesses" in Atspi.Text.get_text(node, 0, -1); stop(pid, aid)
    rows, _ = store.find(dbus.Dictionary({"activity_id": aid}, signature="sv"), dbus.Array(["uid"], signature="s")); assert len(rows) == 1; uid = str(rows[0]["uid"]); filename = Path(str(store.get_filename(uid))); filename.write_text(json.dumps({"secret": ["red", "blue", "yellow", "green"], "guesses": [["red", "red", "red", "red"]], "current": []}) + "\n")
    pid, aid, node = launch(uid, "1 of 6 guesses"); assert "1 of 6 guesses" in Atspi.Text.get_text(node, 0, -1); stop(pid, aid); print("mastermind-roundtrip=PASS resume=PASS cleanup=PASS", flush=True)
