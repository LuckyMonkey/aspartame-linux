#!/usr/bin/env python3
"""Guest regression: resume Implode block grid from Journal."""
import json, os, subprocess, sys, time
from pathlib import Path
BUNDLE_ID = "com.jotaro.ImplodeActivity"; MARKER = "implodeactivity4.ImplodeActivity"
...
def main():
    ...
    def find(node, pid, expected, depth=0):
        ...
        return None
    def launch(uid="", expected="Implode"):
        assert journal.LaunchBundle(BUNDLE_ID, uid); pid, aid = wait_for("process", lambda: next(iter(procs()), None)); wait_for("service", lambda: bus.name_has_owner("org.laptop.Activity" + aid)); assert shell.ActivateActivity(aid); return pid, aid, wait_for("visible Implode", lambda: find(Atspi.get_desktop(0), pid, expected))
    def stop(pid, aid):
    ...
    uid = str(rows[0]["uid"]); filename = Path(str(store.get_filename(uid))); filename.write_text(json.dumps({"blocks": [None, 0, None, 1, None, 2, None, None, None]}) + "\n")
    pid, aid, node = launch(uid, "3 blocks remain"); assert "3 blocks remain" in Atspi.Text.get_text(node, 0, -1); stop(pid, aid)
    print("implode-roundtrip=PASS resume=PASS cleanup=PASS", flush=True)
