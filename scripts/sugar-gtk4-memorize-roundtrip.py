#!/usr/bin/env python3
"""Guest regression: resume Memorize card matches from Journal."""
import json, os, subprocess, sys, time
from pathlib import Path
BUNDLE_ID = "org.laptop.Memorize"; MARKER = "memorizeactivity4.MemorizeActivity"
...
def main():
    ...
    def find(node, pid, expected, depth=0):
        ...
        return None
    def launch(uid="", expected="Memorize"):
        assert journal.LaunchBundle(BUNDLE_ID, uid); pid, aid = wait_for("process", lambda: next(iter(procs()), None)); wait_for("service", lambda: bus.name_has_owner("org.laptop.Activity" + aid)); assert shell.ActivateActivity(aid); return pid, aid, wait_for("visible Memorize", lambda: find(Atspi.get_desktop(0), pid, expected))
    def stop(pid, aid):
    ...
    uid = str(rows[0]["uid"]); filename = Path(str(store.get_filename(uid))); filename.write_text(json.dumps({"cards": [0, 1, 2, 3, 0, 1, 2, 3], "opened": [], "matched": [0, 4]}) + "\n")
    pid, aid, node = launch(uid, "1 of 4 pairs matched"); assert "1 of 4 pairs matched" in Atspi.Text.get_text(node, 0, -1); stop(pid, aid)
    print("memorize-roundtrip=PASS resume=PASS cleanup=PASS", flush=True)
