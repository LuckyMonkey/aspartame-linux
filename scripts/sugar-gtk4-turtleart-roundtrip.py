#!/usr/bin/env python3
"""Guest regression: resume TurtleBlocks position/heading from Journal."""
import json, os, subprocess, sys, time
from pathlib import Path
BUNDLE_ID = "org.laptop.TurtleArtActivity"; MARKER = "turtleartactivity4.TurtleArtActivity"
...
def main():
    ...
    def find(node, pid, expected, depth=0):
        ...
        if node.get_process_id() == pid:
            ...
            if expected in text or expected in name: return node
        ...
        return None
    def launch(uid="", expected="TurtleBlocks"):
        assert journal.LaunchBundle(BUNDLE_ID, uid); pid, aid = wait_for("process", lambda: next(iter(procs()), None)); wait_for("service", lambda: bus.name_has_owner("org.laptop.Activity" + aid)); assert shell.ActivateActivity(aid); return pid, aid, wait_for("visible TurtleBlocks", lambda: find(Atspi.get_desktop(0), pid, expected))
    def stop(pid, aid):
    ...
    uid = str(rows[0]["uid"]); filename = Path(str(store.get_filename(uid))); filename.write_text(json.dumps({"x": 35, "y": -35, "heading": 1.57079632679, "lines": [[[0, 0], [35, 0]]]}) + "\n")
    pid, aid, node = launch(uid, "Position: (35, -35)"); assert "Position: (35, -35)" in Atspi.Text.get_text(node, 0, -1); stop(pid, aid)
    print("turtleart-roundtrip=PASS resume=PASS cleanup=PASS", flush=True)
