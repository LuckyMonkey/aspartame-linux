#!/usr/bin/env python3
"""Guest regression: resume Poll question, choices, and vote counts."""
import json, os, subprocess, sys, time
from pathlib import Path
BUNDLE_ID = "org.worldwideworkshop.PollBuilder"
MARKER = "pollactivity4.PollActivity"
...
def main():
    if "IMAGE_ID=aspartame" not in Path("/etc/os-release").read_text():
        raise SystemExit("Run in guest")
    if os.getuid() == 0:
        pids = subprocess.check_output(
            ["pgrep", "-u", "aspartame", "-f", "/sources/sugar/src/jarabe/main.py"],
            text=True).splitlines()
        if len(pids) != 1:
            raise SystemExit("Expected one modern shell")
    ...
    def find(node, pid, expected, depth=0):
        ...
        return None
    def launch(uid="", expected="What should we explore today?"):
        assert journal.LaunchBundle(BUNDLE_ID, uid)
        pid, aid = wait_for("process", lambda: next(iter(procs()), None))
        wait_for("service", lambda: bus.name_has_owner("org.laptop.Activity" + aid))
        assert shell.ActivateActivity(aid)
        return pid, aid, wait_for("visible Poll", lambda: find(Atspi.get_desktop(0), pid, expected))
    def stop(pid, aid):
        assert shell.StopActivity(aid)
        wait_for("exit", lambda: not Path(f"/proc/{pid}").exists())
        assert not bus.name_has_owner("org.laptop.Activity" + aid)
        assert not shell.ActivateActivity(aid)
    ...
    filename.write_text(json.dumps({"question": "Where next?", "choices": ["Learn", "Build", "Share"], "votes": [2, 0, 1]}) + "\n")
    pid, aid, node = launch(uid, "Where next?")
    count_node = wait_for("visible vote counts", lambda: find(Atspi.get_desktop(0), pid, "2 votes"))
    count_text = Atspi.Text.get_text(count_node, 0, -1)
    assert "2 votes" in count_text
    stop(pid, aid)
    print("poll-roundtrip=PASS resume=PASS cleanup=PASS", flush=True)
