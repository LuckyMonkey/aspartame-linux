...
def get_clock_format():
    settings = Gio.Settings.new("org.aspartame.clock")
    return settings.get_string("format")


def set_clock_format(clock_format):
    if len(clock_format) > 128:
        raise ValueError(_("Clock format is too long."))
    try:
        datetime.now().strftime(clock_format)
    except (TypeError, ValueError):
        raise ValueError(_("Clock format is not valid."))
    settings = Gio.Settings.new("org.aspartame.clock")
    settings.set_string("format", clock_format)
