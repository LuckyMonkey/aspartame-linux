...
class AboutComputer(SectionView):
    ...
    def _setup_system(self):
        ...
        title = Gtk.Label(label=_('System'))
        title.set_alignment(0, 0)
        self._vbox.pack_start(title, False, True, 0)
        title.show()

        box = Gtk.VBox()
        box.set_border_width(style.DEFAULT_SPACING * 2)
        box.set_spacing(style.DEFAULT_SPACING)
        for label, value in self._model.get_system_information():
            box.pack_start(self.create_information_box(label, value),
                           False, True, 0)
        self._vbox.pack_start(box, False, True, 0)
        box.show()
