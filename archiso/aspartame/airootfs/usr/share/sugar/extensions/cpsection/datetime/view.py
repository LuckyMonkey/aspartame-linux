...
class TimeZone(SectionView):
    def __init__(self, model, alerts):
        ...
        self._format_change_handler = 0

        format_label = Gtk.Label(label=_("Clock format"))
        format_label.set_alignment(0, 0.5)
        self.pack_start(format_label, False, False, 0)
        format_help = Gtk.Label(label=_("strftime format; blank follows locale and shows hours and minutes"))
        format_help.set_alignment(0, 0.5)
        self.pack_start(format_help, False, False, 0)
        self._clock_format_entry = Gtk.Entry()
        self._clock_format_entry.set_width_chars(32)
        self._clock_format_entry.set_placeholder_text(_("%H:%M or %I:%M %p"))
        self.pack_start(self._clock_format_entry, False, False, 0)
        self._clock_format_entry.show()
        format_label.show()
        format_help.show()
    ...
    def setup(self):
        ...
        self._clock_format_entry.set_text(self._model.get_clock_format())
        self._format_change_handler = self._clock_format_entry.connect(
            "changed", self.__clock_format_changed_cb)
        self._cursor_change_handler = self._treeview.connect(
            'cursor-changed', self.__zone_changed_cd)

    def apply(self):
        self._model.set_clock_format(self._clock_format_entry.get_text())


    def undo(self):
        self._treeview.disconnect(self._cursor_change_handler)
        self._clock_format_entry.disconnect(self._format_change_handler)
        self._model.undo()
        self._zone_alert.hide()

    def __clock_format_changed_cb(self, entry):
        try:
            datetime.now().strftime(entry.get_text())
