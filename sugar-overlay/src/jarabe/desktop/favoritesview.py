...
class ActivityIcon(CanvasIcon):
    ...
    def __init__(self, activity_info):
        ...
        if aspartame_help is not None:
            self._help_id = aspartame_help.register_activity({
                'id': activity_info.get_bundle_id(),
                'name': activity_info.get_name(),
                'summary': 'Open %s.' % activity_info.get_name(),
            })
            aspartame_help.register_target(self, self._help_id)
            self.set_tooltip_text('Open %s' % activity_info.get_name())
        else:
            self._help_id = None
        self._resume_mode = Gio.Settings.new(
            'org.sugarlabs.user').get_boolean('resume-activity')
    ...
    def __button_activate_cb(self, icon):
        # CanvasIcon owns activation. Intercept at the actual launch boundary
        # so What's This? never opens an Activity by accident.
        if aspartame_help is not None and aspartame_help.is_active():
            aspartame_help._explain_widget(self)
            return
        self._activate()
